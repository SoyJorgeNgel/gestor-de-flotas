<div>
    <div>
        <x-button class="text-white text-sm" wire:click="$set('open' , true)"><i class='bx bxs-plus-square pr-1'></i> Destino</x-button>

        <x-dialog-modal wire:model="open">
            <x-slot name="title">Crear destino</x-slot>
            <x-slot name="content">
                <x-validation-errors class="mb-4" />
                <div>
                    <x-label for="createInstitution" value="Institución" />
                    <x-input class="block mt-1 w-full" wire:model="institution" type="text" id="createInstitution" oninput="this.value = this.value.toUpperCase()" required autofocus />
                </div>
                <div>
